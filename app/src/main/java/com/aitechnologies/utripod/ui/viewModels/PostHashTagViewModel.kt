...
class PostHashTagViewModel(
    ...
    fun getPostByHashtags(hashtag: String) {
        viewModelScope.launch {
            postRepository.getPostsByHashtags(hashtag).collect {
                posts.value = it
            }
        }
    }
    ...
    fun getPostById(id: String) {
        viewModelScope.launch {
            posts.value = postRepository.getPostById(id)
        }
    }
...
class PostHashTagViewModelProvider(
    private val postRepository: PostRepository,
    private val userRepository: UserRepository
) : ViewModelProvider.Factory {
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(PostHashTagViewModel::class.java))
            return PostHashTagViewModel(postRepository, userRepository) as T
